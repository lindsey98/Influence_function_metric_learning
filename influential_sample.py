...
from influence_function import s_test, grad_confusion, grad_alltrain
import pickle
...
if __name__ == '__main__':
    ...
    # for data in dl_ev:
        # z_test ,t_test, _ = data
        # s_test(z_test, t_test, model, criterion, dl_tr)
        # pass

    # calculate all train
    grad4train = grad_alltrain(model, criterion, dl_tr)
    with open("grad_train.pkl", "wb") as fp:  # Pickling
        pickle.dump(grad4train, fp)
    v = torch.load("confusion_grad_101_102.pth")
    h_estimate = s_test(model, criterion, v, dl_tr)
