...
class ProxyNCA_prob_distribution_mixup(torch.nn.Module):
    ...
    def forward(self, X, indices, T):
        ...
        if self.mixup_method == 'none':
            ...
            xSx = torch.matmul(xPdist2, trace_Sigma_inv.T)  # (N+N_inter, C, C)
            ...
            add_sigmas = torch.ones((virtual_proxies.size()[0], self.sz_embed)) # (C', sz_embed)
            trace_Sigma_inv = torch.cat((trace_Sigma_inv, add_sigmas), dim=0) # (C+C', sz_embed)

            xSx = torch.matmul(xPdist2, trace_Sigma_inv.T)  # (N+N_inter, C+C', C+C')
