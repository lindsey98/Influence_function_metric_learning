...
class ProxyNCA_prob(torch.nn.Module):
    ...
    def add_proxy(self, cls, new_proxy):
        ...
        if self.current_proxy[cls] == self.max_proxy_per_class:
            ...
            self.proxies.data[self.max_proxy_per_class * cls + self.current_proxy[cls],
            :] = new_proxy.data  # initilaize new proxy there
    ...
    def inner_product_sim(self, X, P, T):
        ...
        IP_reshape = torch.reshape(IP, (
            X.size(0), self.nb_classes, self.max_proxy_per_class))  # reshape inner product as shape of (N, C, maxP)
    ...
    def forward(self, X, indices, T):
        """
        X: the embedding matrix (batch_size, embedding_size)
        indices: a list of batch indices, a batch index is used to track the graidents back to a sample
        T: a tensor, (batch_size, ), keeping class labels: sample -> class label
        """
        ...
        output = D_reshape.masked_fill_((1 - self.mask).bool(), 0.0)  # something happens here
        prob = F.softmax(-output, dim=-1)  # low distance proxy get higher weights weighted distance
        D_weighted = torch.sum(prob * output, dim=-1)  # reduce to shape (N, C)
