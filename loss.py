...
class ProxyNCA_prob_distribution_mixup(torch.nn.Module):
    '''
        Uncertainty-guided MixUp with Distribion-based loss
    '''
    ...
    def kl_divergence(self, mus, trace_Sigma):
        Sigmas = 1 / trace_Sigma # (C, sz_embed)
        mus = F.normalize(mus, p=2, dim=-1) # (C, sz_embed)
        mus2 = torch.square(mus)
        KL = 0.5*(Sigmas + mus2 - 1 - torch.log(Sigmas)) # (C, sz_embed)
        KL = KL.sum(-1).mean()
        return KL
    ...
    def informative_mixup(self, X, T, IP, P):
        ...
        if self.mixup_method == 'intra':
            # between same class
            virtual_classes_intra, virtual_samples_intra = self.intracls_mixup(X, T, IP, pairs_per_cls=4)
            return virtual_classes_intra, virtual_samples_intra
            ...
            # between dfferent class
            virtual_classes_inter, virtual_samples_inter, virtual_proxies_inter = self.intercls_mixup_proxy(X, T, P, IP)
            return virtual_classes_inter, virtual_samples_inter, virtual_proxies_inter
            ...
            virtual_classes_inter, virtual_samples_inter, virtual_proxies_inter = self.intercls_mixup_proxy(X, T, P, IP)
            return virtual_classes_inter, virtual_samples_inter, virtual_proxies_inter, \
                   virtual_classes_intra, virtual_samples_intra
        else:
            return
    ...
    def forward(self, X, indices, T):
        ...
        if self.mixup_method == 'none':
            ...
            xSx = xSx[:, torch.arange(self.nb_classes), torch.arange(self.nb_classes)]  # (N, C)
            D = xSx  # compute the -(x-mu)'Sigma^-1(x-mu) of shape (N, C)
            logD_sim = F.log_softmax(-D, -1)  # of shape (N+N_intra, C)
            ...
            loss = torch.sum(- T * logD_sim, -1)  # -ylog(yhat)
            loss = loss.mean()
            kl_loss = self.kl_divergence(self.proxies, trace_Sigma_inv)
            loss = loss + 0.5 * kl_loss  # KL regularization on prior N(0, I)
            ...
            xPdist = Xall.unsqueeze(1) - P  # (N+N_intra, C, sz_embed)
            xPdist2 = xPdist * xPdist  # Hadamard product (N+N_intra, C, sz_embed)

            xSx = torch.matmul(xPdist2, trace_Sigma_inv.T)  # (N+N_intra, C, C)
            xSx = xSx[:, torch.arange(self.nb_classes), torch.arange(self.nb_classes)]  # (N+N_intra, C)
            D = xSx  # compute the -(x-mu)'Sigma^-1(x-mu) of shape (N+N_intra, C)
            logD_sim = F.log_softmax(-D, -1)  # of shape (N+N_intra, C)
            ...
            loss = torch.sum(- Tall * logD_sim, -1)  # -ylog(yhat)
            loss = loss.mean()
            kl_loss = self.kl_divergence(self.proxies, trace_Sigma_inv)
            loss = loss + 0.5 * kl_loss  # KL regularization on prior N(0, I)
            ...
            xPdist = Xall.unsqueeze(1) - P  # (N+N_inter, C, sz_embed)
            xPdist2 = xPdist * xPdist  # Hadamard product (N+N_inter, C, sz_embed)

            xSx = torch.matmul(xPdist2.float(), trace_Sigma_inv.T.float())  # (N+N_inter, C, C)
            xSx = xSx[:, torch.arange(self.nb_classes), torch.arange(self.nb_classes)]  # (N+N_inter, C)
            D = xSx  # compute the -(x-mu)'Sigma^-1(x-mu) of shape (N+N_inter, C)
            logD_sim = F.log_softmax(-D, -1)  # of shape (N+N_inter, C)
            ...
            xPdist = Xall.unsqueeze(1) - Pall  # (N+N_inter, C+C', sz_embed)
            xPdist2 = xPdist * xPdist  # Hadamard product (N+N_inter, C+C', sz_embed)

            add_sigmas = torch.ones((virtual_proxies.size()[0], self.sz_embed)).to(trace_Sigma_inv.device) # (C', sz_embed)
            add_sigmas.requires_grad = False
            trace_Sigma_inv = torch.cat((trace_Sigma_inv, add_sigmas), dim=0) # (C+C', sz_embed)

            xSx = torch.matmul(xPdist2.float(), trace_Sigma_inv.T.float())  # (N+N_inter, C+C', C+C')
            xSx = xSx[:, torch.arange(self.nb_classes+virtual_proxies.size()[0]), torch.arange(self.nb_classes+virtual_proxies.size()[0])]  # (N+N_inter, C+C')
            D = xSx  # compute the -(x-mu)'Sigma^-1(x-mu) of shape (N+N_inter, C+C')
            logD_sim = F.log_softmax(-D, -1)  # of shape (N+N_inter, C+C')

            loss = torch.sum(- Tall * logD_sim, -1)  # -ylog(yhat)
            loss = loss.mean()
            kl_loss = self.kl_divergence(Pall, trace_Sigma_inv)
            loss = loss + 0.5 * kl_loss  # KL regularization on prior N(0, I)

        elif self.mixup_method == 'both':
            '''Both intra-class and inter-class'''
            virtual_classes_inter, virtual_samples_inter, virtual_proxies_inter, \
            virtual_classes_intra, virtual_samples_intra = self.informative_mixup(X=X, T=T, IP=IP, P=P)

            Xall = torch.cat((X, self.scale * virtual_samples_inter, self.scale * virtual_samples_intra),
                             dim=0)  # (N+N_inter+N_intra, sz_embed)
            Tall = torch.cat([T, virtual_classes_inter, virtual_classes_intra], 0)  # (N+N_inter+N_intra, )
            Pall = torch.cat([P, self.scale * virtual_proxies_inter])  # (C+C_inter, sz_embed)
            ...
            add_sigmas = torch.ones((virtual_proxies_inter.size()[0], self.sz_embed)) # (C', sz_embed)
            trace_Sigma_inv = torch.cat((trace_Sigma_inv, add_sigmas), dim=0) # (C+C', sz_embed)

            xSx = torch.matmul(xPdist2, trace_Sigma_inv.T.double())  # (N+N_inter, C+C', C+C')
            xSx = xSx[:, torch.arange(self.nb_classes+virtual_proxies_inter.size()[0]), torch.arange(self.nb_classes+virtual_proxies_inter.size()[0])]  # (N+N_inter, C+C')
            D = xSx  # compute the -(x-mu)'Sigma^-1(x-mu) of shape (N+N_inter, C+C')
            logD_sim = F.log_softmax(-D, -1)  # of shape (N+N_inter, C+C')
