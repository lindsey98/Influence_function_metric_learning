...
if __name__ == '__main__':
    ...
    plot_dir = 'dvi_data_{}/resnet_2048_umap_plots'.format(dataset_name)

    sz_embedding = 2048

    os.makedirs(plot_dir, exist_ok=True)
    dl_tr, dl_ev = prepare_data(data_name=dataset_name, root='dvi_data_{}/'.format(dataset_name), save=False)
    ...
    with open("{0}/{1}_ip.json".format('log', '{}_{}_trainval_2048_0'.format(dataset_name, dataset_name)), 'rt') as handle:
    ...
    for e in tqdm(range(40)):
        ...
        embedder.fit_transform(np.concatenate((embedding.detach().cpu().numpy(), proxies.cpu().numpy()), axis=0))
        ...
        plt.scatter(low_dim_proxy[:, 0], low_dim_proxy[:, 1], c=range(10), cmap='tab10', marker=(5,1), s=8)
