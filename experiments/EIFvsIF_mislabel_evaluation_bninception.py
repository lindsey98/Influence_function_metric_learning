...
if __name__ == '__main__':
    noisy_level = 0.1
    sz_embedding = 512; epoch = 40; test_crop = False; model_arch = 'BnInception'
    # loss_type = 'ProxyNCA_prob_orig_noisy_{}'.format(noisy_level); dataset_name = 'cub_noisy';  config_name = 'cub_ProxyNCA_prob_orig_bninception'; seed = 0
    # loss_type = 'ProxyNCA_prob_orig_noisy_{}'.format(noisy_level); dataset_name = 'cars_noisy'; config_name = 'cars_ProxyNCA_prob_orig_bninception'; seed = 0
    loss_type = 'ProxyNCA_prob_orig_noisy_{}'.format(noisy_level); dataset_name = 'inshop_noisy'; config_name = 'inshop_ProxyNCA_prob_orig_bninception'; seed = 0

    # loss_type = 'SoftTriple_noisy_{}'.format(noisy_level); dataset_name = 'cub_noisy';  config_name = 'cub_SoftTriple_bninception'; seed = 3
    # loss_type = 'SoftTriple_noisy_{}'.format(noisy_level); dataset_name = 'cars_noisy';  config_name = 'cars_SoftTriple_bninception'; seed = 4
    # loss_type = 'SoftTriple_noisy_{}'.format(noisy_level); dataset_name = 'inshop_noisy';  config_name = 'inshop_SoftTriple_bninception'; seed = 3

    '''============================================= Our Empirical Influence function =============================================================='''
    IS = EIF(dataset_name, seed, loss_type, config_name, data_transform_config='dataset/config_bninception.json',
             test_crop=test_crop, sz_embedding=sz_embedding, epoch=epoch, model_arch=model_arch)
    basedir = 'MislabelExp_Influential_data_BnInception'
    ...
    for num_thetas in [1, 2, 3]:
        ...
        training_sample_by_influence = influence_values.argsort()
        ...
        plt.plot(cum_overlap, label='EIF with $N\_theta$ = {}'.format(num_thetas))
    ...
    IS = OrigIF(dataset_name, seed, loss_type, config_name, data_transform_config='dataset/config_bninception.json',
                test_crop=test_crop, sz_embedding=sz_embedding, epoch=epoch, model_arch=model_arch)
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig('./images/mislabel_{}_{}_alltheta_noisylevel{}_BnInception.pdf'.format(dataset_name, loss_type, noisy_level),
                bbox_inches='tight')
