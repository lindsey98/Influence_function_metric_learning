...
if __name__ == '__main__':
    ...
    IS = EIF(dataset_name, seed, loss_type, config_name, 'dataset/config.json', test_crop, sz_embedding, epoch, 'ResNet', 0.01)
    ...
    for num_thetas in [1]:
        ...
        if os.path.exists("{}/{}_{}_helpful_testcls{}_SIF_theta{}_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, num_thetas, noisy_level)):
            helpful_indices = np.load("{}/{}_{}_helpful_testcls{}_SIF_theta{}_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, num_thetas, noisy_level))
            harmful_indices = np.load("{}/{}_{}_harmful_testcls{}_SIF_theta{}_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, num_thetas, noisy_level))
            influence_values = np.load("{}/{}_{}_influence_values_testcls{}_SIF_theta{}_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, num_thetas, noisy_level))
        else:
            ...
            np.save("{}/{}_{}_helpful_testcls{}_SIF_theta{}_{}_debug".format(basedir, IS.dataset_name, IS.loss_type, 0, num_thetas, noisy_level), helpful_indices)
            np.save("{}/{}_{}_harmful_testcls{}_SIF_theta{}_{}_debug".format(basedir, IS.dataset_name, IS.loss_type, 0, num_thetas, noisy_level), harmful_indices)
            np.save("{}/{}_{}_influence_values_testcls{}_SIF_theta{}_{}_debug".format(basedir, IS.dataset_name, IS.loss_type, 0, num_thetas, noisy_level), influence_values)

        # mislabelled indices ground-truth
        training_sample_by_influence = np.abs(influence_values).argsort()[::-1]  # fixme: abs value
        # training_sample_by_influence = influence_values.argsort()[::-1]  # fixme: harmful is positive
        # training_sample_by_influence = influence_values.argsort()  # fixme

        gt_mislabelled_indices = IS.dl_tr.dataset.noisy_indices
        overlap_EIF = np.isin(training_sample_by_influence, gt_mislabelled_indices)
        cum_overlap = np.cumsum(overlap_EIF)
    ...
    harmful_indices = np.load("{}/{}_{}_harmful_testcls{}_SIF_theta{}_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, 1, noisy_level))
    relabel_dict = {}
    ...
    IS = OrigIF(dataset_name, seed, loss_type, config_name, 'dataset/config.json', test_crop, sz_embedding, epoch, 'ResNet', 0.01)
    basedir = 'MislabelExp_Influential_data'
    os.makedirs(basedir, exist_ok=True)

    '''Mislabelled data detection'''
    if os.path.exists("{}/{}_{}_helpful_testcls{}_IF_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, noisy_level)):
        helpful_indices = np.load("{}/{}_{}_helpful_testcls{}_IF_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, noisy_level))
        harmful_indices = np.load("{}/{}_{}_harmful_testcls{}_IF_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, noisy_level))
        influence_values = np.load("{}/{}_{}_influence_values_testcls{}_IF_{}_debug.npy".format(basedir, IS.dataset_name, IS.loss_type, 0, noisy_level))
    else:
        ...
        np.save("{}/{}_{}_helpful_testcls{}_IF_{}_debug".format(basedir, IS.dataset_name, IS.loss_type, 0, noisy_level), helpful_indices)
        np.save("{}/{}_{}_harmful_testcls{}_IF_{}_debug".format(basedir, IS.dataset_name, IS.loss_type, 0, noisy_level), harmful_indices)
        np.save("{}/{}_{}_influence_values_testcls{}_IF_{}_debug".format(basedir, IS.dataset_name, IS.loss_type, 0, noisy_level), influence_values)
    ...
    gt_mislabelled_indices = IS.dl_tr.dataset.noisy_indices
    overlap_IF = np.isin(training_sample_by_influence, gt_mislabelled_indices)
    cum_overlap = np.cumsum(overlap_IF)

    plt.plot(cum_overlap, label='IF')

    '''======================================================================================================================================='''
    np.sum(overlap_IF[:int(len(overlap_IF)*0.3)]) / len(gt_mislabelled_indices)
    '''=============================================Random================================================================'''
    overlap_random = np.isin(np.arange(len(IS.dl_tr.dataset)), gt_mislabelled_indices)
    cum_overlap = np.cumsum(overlap_random)

    plt.plot(cum_overlap, label='random')
    plt.legend()
    plt.tight_layout()
    plt.show()
    # plt.savefig('./images/mislabel_{}_{}_alltheta_noisylevel{}_debug.pdf'.format(dataset_name, loss_type, noisy_level),
    #             bbox_inches='tight')
